    ...
    try {
        const query = `select U.id,
                       ...
                       from user U
                                ...
                                left join (select distinct customerCode, customerName from customer) C
...
exports.verifyEmailOTP = async (req, next) => {
    try {
        ...
        const query = `SELECT DISTINCT customer.Id,
                                       customer.CustomerName,
                                       customer.CustomerCode,
                                       GROUP_CONCAT(categories.name) AS Categories
                       FROM logione.customer
                                JOIN logione.categories ON customer.categoryId = categories.categoryId
                       WHERE customer.CustomerCode = ${req.user.customerCode}
